...
def split_domain(domain_dir: str, out_dir: str, prefix: str, train_ratio: float = 0.8):
    """
    domain_dir 에 있는 .png/.jpg/.jpeg 파일을 읽어서
    train_ratio 비율만큼 train_<prefix>.txt, 나머지를 test_<prefix>.txt 에
    '파일명만' 기록합니다 (절대경로 제거).
    """
    # 1) 파일 목록 수집
    imgs = [
    ...
    if not imgs:
        print(f"⚠️  {domain_dir} 에는 이미지 파일이 없습니다.")
        return
...
if __name__ == "__main__":
    ...
    domain_B = os.path.join(base, "B")

    print(f"▶️ base       = {base}")
    print(f"▶️ domain_dir = {domain_B}\n")

    split_domain(domain_B, base, "B")
